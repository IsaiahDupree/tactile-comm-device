...
int16_t idxForKey(const String& key) {
  ...
  for (uint16_t i = 0; i < TOTAL_BUTTONS; i++) {
    if (mapTab[i].used && strcasecmp(mapTab[i].label, key.c_str()) == 0) {
      return i;
    }
  }
  return -1;
...
// FS_STAGING_DIR now defined in main file
bool maintenanceMode = false;

/**
 * Initializes the file system session system
 */
// initFSSession moved to main file to prevent duplication
...
void cmd_FS_COMMIT() {
  ...
  while (entry = dir.openNextFile()) {
    ...
    // Copy staging file to destination (workaround since SD.rename isn't available on R4 WiFi)
    File sourceFile = SD.open(sourcePath, FILE_READ);
    if (!sourceFile) {
      Serial.print(F("ERR OPEN_FAILED "));
    ...
    if (!destFile) {
      sourceFile.close();
      Serial.print(F("ERR CREATE_FAILED "));
    ...
    while (sourceFile.available()) {
      int bytesRead = sourceFile.read(buffer, bufSize);
      if (bytesRead > 0) {
    ...
    if (!SD.remove(sourcePath)) {
      Serial.print(F("WARNING: Failed to remove source file "));
      Serial.println(sourcePath);
