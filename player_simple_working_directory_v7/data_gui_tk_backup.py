...
class App(tk.Tk):
    ...
    def _apply_enable(self, enabled):
        for b in (self.btn_ls, self.btn_put, self.btn_get, self.btn_del,
                  ...
                  self.btn_flag_on, self.btn_flag_off):
    ...
    def _busy(self, on=True):
        if on:
            self.pbar.config(mode="indeterminate"); self.pbar.start(10)
        else:
            self.pbar.stop(); self.pbar.config(mode="determinate"); self.pbar["value"]=0
    ...
    def _ls(self):
        ...
        def ok(items):
            ...
            self._busy(False); self._log(f"[ok] {len(items)} item(s)")

        def err(e):
            self._busy(False); self._log(f"[err] ls: {e}")
    ...
    def _exit_data_mode(self):
        ...
        def ok(_):
            self._busy(False); self._log("[ok] exited data mode")
            self._disconnect()

        def err(e):
            self._busy(False); self._log(f"[err] exit: {e}")
    ...
    def _flag_on(self):
        ...
        def ok(_): self._busy(False); self._log("[ok] writes enabled")
        def err(e): self._busy(False); self._log(f"[err] flag on: {e}")
        self._run_bg(task, ok, err)
    ...
    def _flag_off(self):
        ...
        def ok(_): self._busy(False); self._log("[ok] writes disabled")
        def err(e): self._busy(False); self._log(f"[err] flag off: {e}")
        self._run_bg(task, ok, err)
    ...
    def _put(self):
        ...
        def task(): return cli.cmd_put(self.ser, bank, key, dest, path, use_crc=True)

        def ok(_):
            self._busy(False); self._log("[ok] upload complete")

        def err(e):
            self._busy(False); self._log(f"[err] put: {e}")
    ...
    def _get(self):
        ...
        def task(): return cli.cmd_get(self.ser, bank, key, name, outpath)

        def ok(_):
            self._busy(False); self._log("[ok] download complete")

        def err(e):
            self._busy(False); self._log(f"[err] get: {e}")
    ...
    def _delete(self):
        ...
        def ok(_):
            self._busy(False); self._log("[ok] delete done"); self._ls()

        def err(e):
            self._busy(False); self._log(f"[err] del: {e}")
    ...
    def _sync_seq(self):
        ...
        def ok(_):
            self._busy(False); self._log("[ok] sync seq complete")

        def err(e):
            self._busy(False); self._log(f"[err] sync seq: {e}")
    ...
    def _sync_pre(self):
        ...
        def ok(_):
            self._busy(False); self._log("[ok] sync preserve complete")

        def err(e):
            self._busy(False); self._log(f"[err] sync preserve: {e}")
