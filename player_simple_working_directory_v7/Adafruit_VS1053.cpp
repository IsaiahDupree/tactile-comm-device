...
Adafruit_VS1053::Adafruit_VS1053(int8_t mosi, int8_t miso, int8_t clk,
  ...
  misoportreg = (PortReg *)portInputRegister(digitalPinToPort(_miso));  // ← cast away const
...
void Adafruit_VS1053::sineTest(uint8_t n, uint16_t ms) {
  ...
  // Wait for DREQ with a timeout to avoid getting stuck if codec isn't ready
  unsigned long _sine_wait_start = millis();
  while (!digitalRead(_dreq)) {
    if (millis() - _sine_wait_start > 500) { // ~0.5s timeout
      Serial.println(F("[SINE] DREQ not ready, aborting sine test"));
      return;
    }
    delay(1);
  }
