...
// Find or create playlist cache entry for a key
PlaylistCache* getPlaylistCache(const char* key) {
  // First check if already cached
  for (uint8_t i = 0; i < cachedPlaylistCount; i++) {
    if (strcmp(playlistCache[i].key, key) == 0) {
      return &playlistCache[i];
    }
  }
  
  // Create new cache entry if space available
...
// Load M3U playlist from SD card
bool loadPlaylist(const char* key, bool isHuman, Playlist* playlist) {
  if (playlist->loaded) return true; // Already loaded
  
  // Build playlist path
  char playlistPath[64];
  snprintf(playlistPath, sizeof(playlistPath), "/mappings/playlists/%s_%s.m3u", 
           key, isHuman ? "human" : "generated");
  
  Serial.print(F("[PLAYLIST] Loading "));
  ...
  if (entryCount == 0) {
    Serial.println(F("[PLAYLIST] No valid entries found"));
    return false;
  }
  ...
  if (!playlist->entries) {
    Serial.println(F("[PLAYLIST] Failed to allocate memory"));
    return false;
  }
  ...
  Serial.print(idx);
  Serial.print(F(" entries for "));
  Serial.println(key);
  
  return true;
}

// Get next track from playlist based on priority mode
const char* getNextTrack(const char* key, uint8_t pressCount) {
  if (!modeCfg.strictPlaylists) {
    return nullptr; // Fall back to legacy system
  }
  
  PlaylistCache* cache = getPlaylistCache(key);
  if (!cache) {
    Serial.println(F("[PLAYLIST] Cache full, cannot load playlist"));
    return nullptr;
  }
  
  // Determine which bank to use based on priority and press count
  bool useHuman;
  if (modeCfg.priority == GlobalPriority::HUMAN_FIRST) {
    useHuman = true; // Try human first
  } else {
    useHuman = false; // Try generated first
  }
  
  Playlist* primaryPlaylist = useHuman ? &cache->humanPlaylist : &cache->generatedPlaylist;
  Playlist* fallbackPlaylist = useHuman ? &cache->generatedPlaylist : &cache->humanPlaylist;
  
  // Load primary playlist if needed
  if (!loadPlaylist(key, useHuman, primaryPlaylist)) {
    Serial.println(F("[PLAYLIST] Primary playlist failed to load, trying fallback"));
    // Try fallback
    if (!loadPlaylist(key, !useHuman, fallbackPlaylist)) {
      Serial.println(F("[PLAYLIST] Both playlists failed to load"));
      return nullptr;
    }
    primaryPlaylist = fallbackPlaylist;
  }
  
  if (primaryPlaylist->count == 0) {
    Serial.println(F("[PLAYLIST] No tracks in playlist"));
    return nullptr;
  }
  
  // Handle multi-press: advance cursor for single press, select specific index for multi-press
  if (pressCount == 1) {
    // Single press: advance cursor
    primaryPlaylist->cursor = (primaryPlaylist->cursor + 1) % primaryPlaylist->count;
  } else if (pressCount > 1) {
    // Multi-press: select specific index (1-based to 0-based)
    uint8_t targetIndex = (pressCount - 1) % primaryPlaylist->count;
    primaryPlaylist->cursor = targetIndex;
  }
  
  return primaryPlaylist->entries[primaryPlaylist->cursor].path;
}

// Clear playlist cache (for reloading)
...
void loadConfig() {
  ...
  while (cfg.available()) {
    ...
    lbl.trim();
    if (idx < TOTAL_BUTTONS && lbl.length()) {
      lbl.toCharArray(mapTab[idx].label, sizeof(mapTab[idx].label));
      mapTab[idx].used = true;
    }
  }
  ...
  for (uint8_t i = 0; i < TOTAL_BUTTONS; i++) {
    if (mapTab[i].used) {
      if (i < TOTAL_EXPANDER_PINS) {
        ...
        uint8_t xp = extraPins[i - TOTAL_EXPANDER_PINS];
        Serial.print(F("Pin "));
        Serial.print(xp);
      }
      Serial.print(F(" → "));
...
// Announce priority mode change with voice feedback
void announcePriorityMode(PriorityMode mode) {
  Serial.print(F("[MODE] Priority mode: "));
  Serial.println((mode == HUMAN_FIRST) ? F("HUMAN_FIRST") : F("GENERATED_FIRST"));
  
  // Play voice announcement
  const char* modeFile = (mode == HUMAN_FIRST) ? "/33/001.mp3" : "/33/002.mp3";
  if (SD.exists(modeFile)) {
    musicPlayer.startPlayingFile(modeFile);
  }
}
...
// Priority mode integration complete - using existing functions with enhancements
  if (Serial.available()) {
    ...
    // Check for FS protocol commands first
    if (input.startsWith("FS_") || input == "CFG_RELOAD" || input == "GET_INFO") {
      processFSCommand(input);
      return;
    }
    ...
    if (input.length() == 1) {
      ...
      case 'X': case 'x':
        ...
        break;
      case 'V': case 'v':
        Serial.println(F("Volume: +/- or 1-9 (1=max, 9=quiet)"));
        break;
      case '+':
        musicPlayer.setVolume(1, 1);
      ...
      case 'C': case 'c':
        ...
        Serial.print(F("[CALIBRATE] Mode "));
        Serial.println(inCalibrate ? F("ENABLED") : F("DISABLED"));
        if (inCalibrate) printCalibrationInstructions();
        break;
      case 'E': case 'e':
        // Erase all mappings
        ...
        Serial.println(F("[CALIBRATE] All mappings erased"));
        break;
      case 'R': case 'r':
#ifdef FEATURE_SD_PLAYLISTS
        // Reload playlists and configuration
        clearPlaylistCache();
        loadModeCfg();
        Serial.println(F("[CONFIG] Playlists and configuration reloaded"));
#else
        Serial.println(F("[CONFIG] Playlist feature not enabled"));
#endif
        break;
      }
    }
  ...
  // Handle multi-press timeouts
  handleMultiPress();
  
  // Finalize period triple-press window
  finalizePeriodWindow();
...
void handlePress(uint8_t idx) {
  ...
  if (now - lastPressTime[idx] < MULTI_PRESS_WINDOW) {
    pressCount[idx]++;
  } else {
    pressCount[idx] = 1;
  }
  ...
  // Get button label
  const char* label = getButtonLabel(idx);
  
  if (inCalibrate) {
    // Calibration mode - assign label
    Serial.print(F("[CALIBRATE] Button "));
    Serial.print(idx);
    Serial.print(F(" pressed. Enter label (or press Enter to skip): "));
    
    // Wait for serial input
    String input = "";
    unsigned long timeout = millis() + 10000; // 10 second timeout
    while (millis() < timeout && input.length() == 0) {
      if (Serial.available()) {
    ...
    if (input.length() > 0) {
      strncpy(mapTab[idx].label, input.c_str(), sizeof(mapTab[idx].label) - 1);
      mapTab[idx].label[sizeof(mapTab[idx].label) - 1] = '\0';
      mapTab[idx].used = true;
      Serial.print(F("[CALIBRATE] Button "));
      Serial.print(idx);
      Serial.print(F(" assigned label: "));
      Serial.println(input);
    } else {
      Serial.println(F("[CALIBRATE] Skipped"));
    }
    return;
  }
  
  if (!label || strlen(label) == 0) {
    Serial.print(F("[PRESS] Unmapped button "));
    Serial.println(idx);
    return;
  }
  ...
  Serial.println(pressCount[idx]);
  
  // Handle PERIOD button triple-press for mode switching
  if (strcmp(label, "PERIOD") == 0) {
    handlePeriodPress();
  }
  
  // Play audio with playlist engine integration
  playButtonAudio(label, pressCount[idx]);
}
...
void playButtonAudio(const char* label, uint8_t pressCount) {
  ...
  if (modeCfg.strictPlaylists) {
    // Use playlist engine
    const char* trackPath = getNextTrack(label, pressCount);
    if (trackPath) {
  ...
  bool useRecorded = (currentMode == HUMAN_FIRST);
